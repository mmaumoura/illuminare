<x-app-layout>

    <div class="row mb-3 align-items-center">
        <div class="col">
            <h2 class="page-title">Produtos</h2>
            <p class="text-muted mt-1">Gerencie os produtos disponíveis nas unidades.</p>
        </div>
        <div class="col-auto">
            <a href="{{ route('comercial.produtos.create') }}" class="btn btn-primary">
                <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24" fill="none"
                     stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                    <path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M12 5l0 14"/><path d="M5 12l14 0"/>
                </svg>
                Novo Produto
            </a>
        </div>
    </div>

    @if(session('success'))
    <div class="alert alert-success alert-dismissible" role="alert">
        {{ session('success') }}
        <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
    </div>
    @endif

    <div class="card">
        <div class="card-body border-bottom py-3">
            <div class="d-flex align-items-center">
                <div class="text-secondary">
                    Exibir
                    <div class="mx-2 d-inline-block">
                        <span>{{ $products->perPage() }}</span>
                    </div>
                    resultados por página
                </div>
                <div class="ms-auto text-secondary d-flex align-items-center gap-2">
                    Pesquisar:
                    <form method="GET" action="{{ route('comercial.produtos.index') }}" class="d-flex gap-2">
                        <input type="text" name="search" value="{{ request('search') }}"
                               class="form-control form-control-sm" placeholder="Buscar registros">
                        @if(request('search'))
                        <a href="{{ route('comercial.produtos.index') }}" class="btn btn-sm btn-outline-secondary">✕</a>
                        @endif
                    </form>
                </div>
            </div>
        </div>
        <div class="table-responsive">
            <table class="table table-vcenter card-table">
                <thead>
                    <tr>
                        <th>Código</th>
                        <th>Nome</th>
                        <th>Categoria</th>
                        <th>Preço Venda</th>
                        <th>Estoque</th>
                        <th>Status</th>
                        <th class="w-1">Ações</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($products as $product)
                    <tr>
                        <td class="text-muted">{{ $product->code ?? '—' }}</td>
                        <td>
                            <div class="d-flex align-items-center gap-2">
                                @if($product->image_path)
                                <span class="avatar avatar-sm rounded"
                                      style="background-image:url({{ Storage::url($product->image_path) }})"></span>
                                @else
                                <span class="avatar avatar-sm rounded bg-blue-lt text-blue">
                                    {{ strtoupper(substr($product->name, 0, 1)) }}
                                </span>
                                @endif
                                <div>
                                    <div class="fw-semibold">{{ $product->name }}</div>
                                    @if($product->brand)
                                    <div class="text-muted small">{{ $product->brand }}</div>
                                    @endif
                                </div>
                            </div>
                        </td>
                        <td class="text-muted">{{ $product->category ?? '—' }}</td>
                        <td>R$ {{ number_format($product->sale_price, 2, ',', '.') }}</td>
                        <td>
                            @if($product->controls_stock)
                                @if($product->is_low_stock)
                                <span class="text-danger fw-semibold">{{ $product->stock_current }}</span>
                                <span class="badge bg-danger-lt ms-1">baixo</span>
                                @else
                                <span class="text-success fw-semibold">{{ $product->stock_current }}</span>
                                @endif
                            @else
                            <span class="text-muted">—</span>
                            @endif
                        </td>
                        <td>
                            @php $color = $product->status === 'Ativo' ? 'success' : 'secondary'; @endphp
                            <span class="badge bg-{{ $color }}-lt text-{{ $color }}">{{ $product->status }}</span>
                        </td>
                        <td>
                            <div class="d-flex gap-1">
                                <a href="{{ route('comercial.produtos.show', $product) }}"
                                   class="btn btn-sm btn-ghost-secondary" title="Ver">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-sm" width="24" height="24"
                                         viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                         stroke-linecap="round" stroke-linejoin="round">
                                        <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
                                        <circle cx="12" cy="12" r="2"/>
                                        <path d="M22 12c-2.667 4.667 -6 7 -10 7s-7.333 -2.333 -10 -7c2.667 -4.667 6 -7 10 -7s7.333 2.333 10 7"/>
                                    </svg>
                                </a>
                                <a href="{{ route('comercial.produtos.edit', $product) }}"
                                   class="btn btn-sm btn-ghost-secondary" title="Editar">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-sm" width="24" height="24"
                                         viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                         stroke-linecap="round" stroke-linejoin="round">
                                        <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
                                        <path d="M7 7h-1a2 2 0 0 0 -2 2v9a2 2 0 0 0 2 2h9a2 2 0 0 0 2 -2v-1"/>
                                        <path d="M20.385 6.585a2.1 2.1 0 0 0 -2.97 -2.97l-8.415 8.385v3h3l8.385 -8.415z"/>
                                    </svg>
                                </a>
                                <form method="POST" action="{{ route('comercial.produtos.destroy', $product) }}"
                                      onsubmit="return confirm('Excluir este produto?')">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="btn btn-sm btn-ghost-danger" title="Excluir">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-sm" width="24" height="24"
                                             viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                             stroke-linecap="round" stroke-linejoin="round">
                                            <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
                                            <path d="M4 7l16 0"/><path d="M10 11l0 6"/><path d="M14 11l0 6"/>
                                            <path d="M5 7l1 12a2 2 0 0 0 2 2h8a2 2 0 0 0 2 -2l1 -12"/>
                                            <path d="M9 7v-3a1 1 0 0 1 1 -1h4a1 1 0 0 1 1 1v3"/>
                                        </svg>
                                    </button>
                                </form>
                            </div>
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="7" class="text-center text-muted py-4">Nenhum produto encontrado.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        @if($products->hasPages())
        <div class="card-footer d-flex align-items-center">
            {{ $products->links() }}
        </div>
        @endif
    </div>

</x-app-layout>
